...
|ct_inverse_mod_256| PROC
	...
	mov	x16, x12			// exact |f0|
	mov	x17, x13			// exact |g0|
	mov	x12, x14
	mov	x13, x15
	add	x0, x0, #8*8	// pointer to dst |u|
	bl	__smul_256x63
	adc	x22, x22, x23
	str	x22, [x0,#8*4]

	mov	x16, x12			// exact |f1|
	mov	x17, x13			// exact |g1|
	add	x0, x0, #8*5	// pointer to dst |v|
	bl	__smul_256x63
	bl	__smul_512x63_tail
	...
	and	x8, x8, x19		// subtract mod<<256 conditionally
	and	x9, x9, x19
	subs	x4, x4, x8
	and	x10, x10, x19
	sbcs	x5, x5, x9
...
|__smul_256_n_shift_by_62| PROC
	...
	ALIGN	16
|__ab_approximation_62_256| PROC
	ldp	x6, x7, [x1,#8*2]
	ldp	x10, x11, [x1,#8*6]
	ldp	x4, x5, [x1,#8*0]
	ldp	x8, x9, [x1,#8*4]
