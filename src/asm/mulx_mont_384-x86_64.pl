...
.cfi_end_prologue
	...
	lea	40(%rsp), $b_org	# t0
	mov	8*1(%rsp), $n_ptr
	call	__${pre}__sub_mod_384x384	# t2-t0

	lea	($r_ptr), $a_ptr	# t2
	lea	-96($r_ptr), $b_org	# t1
	call	__${pre}__sub_mod_384x384	# t2-t0-t1
	...
	call	__${pre}__mulx_by_1_mont_384
...
.cfi_end_prologue
	...
	mov	$b_org, $n_ptr		# n_ptr
					# gap for __${pre}__mulx_mont_384
	mov	$r_ptr, 8*2(%rsp)
	mov	$a_ptr, 8*3(%rsp)
	...
	mulx	%r14, %r8, %r9
	call	__${pre}__mulx_mont_384
___
{
my @acc = map("%r$_","dx",15,"ax",12,"di","bp",	# output from __${pre}__mulx_mont_384
                      8..11,13,14);
$code.=<<___;
...
.type	__${pre}__mulx_384,\@abi-omnipotent
.align	32
__${pre}__mulx_384:
	mov	8*0($b_ptr), %rdx
	mov	8*0($a_ptr), $a0
...
$code.=<<___;
	...
	ret
.size	__${pre}__mulx_384,.-__${pre}__mulx_384
___
}
{ ########################################################## 384-bit sqrx
$code.=<<___;
...
$code.=<<___;
.type	__${pre}__sqrx_384,\@abi-omnipotent
.align	32
__${pre}__sqrx_384:
	mov	8*0($a_ptr), %rdx
	mov	8*1($a_ptr), @acc[7]
...
$code.=<<___;
	...
	ret
.size	__${pre}__sqrx_384,.-__${pre}__sqrx_384
___
} else {
...
$code.=<<___;
.type	__${pre}__mulx_by_1_mont_384,\@abi-omnipotent
.align	32
__${pre}__mulx_by_1_mont_384:
	mov	8*0($a_ptr), @acc[0]
	mov	$n0, %rdx
	mov	8*1($a_ptr), @acc[1]
	mov	8*2($a_ptr), @acc[2]
...
$code.=<<___;
	ret
.size	__${pre}__mulx_by_1_mont_384,.-__${pre}__mulx_by_1_mont_384
...
$code.=<<___;
.type	__${pre}__mulx_mont_384,\@abi-omnipotent
.align	32
__${pre}__mulx_mont_384:
.cfi_startproc
...
$code.=<<___;
	...
	ret
.cfi_endproc
.size	__${pre}__mulx_mont_384,.-__${pre}__mulx_mont_384
___
}
$code.=<<___;
.globl	sqrx_mont_384
...
.cfi_end_prologue
	...
	call	__${pre}__mulx_mont_384		# as fast as dedicated squaring
...
.cfi_end_prologue
	...
	mov	$a_ptr, $b_ptr
	mov	8*3($a_ptr), @acc[4]
	mov	$r_ptr, 8*2(%rsp)	# to __${pre}__mulx_mont_384
	mov	8*4($a_ptr), $lo
	mov	8*5($a_ptr), $hi
...
.cfi_end_prologue
	...
	mov	$a_ptr, $b_ptr
	mov	8*3($a_ptr), @acc[4]
	mov	$r_ptr, 8*2(%rsp)	# to __${pre}__mulx_mont_383_nonred
	mov	8*4($a_ptr), $lo
	mov	8*5($a_ptr), $hi
...
.Loop_sqrx_383:
	...
	mulx	%rdx, @acc[0], @acc[1]	# a[0]*a[0]
	call	__${pre}__mulx_mont_383_nonred	# omitting full reduction gives ~15%
					# in addition-chains
	movd	%xmm1, @acc[2]d
	dec	@acc[2]d
	jnz	.Loop_sqrx_383
...
$code.=<<___;
.type	__${pre}__mulx_mont_383_nonred,\@abi-omnipotent
.align	32
__${pre}__mulx_mont_383_nonred:
.cfi_startproc
	mulx	@acc[7], @acc[6], @acc[2]
	mulx	@acc[8], @acc[7], @acc[3]
	add	@acc[6], @acc[1]
...
$code.=<<___;
	...
	ret
.cfi_endproc
.size	__${pre}__mulx_mont_383_nonred,.-__${pre}__mulx_mont_383_nonred
___
} } }
{ my $frame = 4*8 +	# place for argument off-load +
	      2*384/8 +	# place for 2 384-bit temporary vectors
...
.cfi_end_prologue
	...
	mulx	%r14, %r8, %r9
	call	__${pre}__mulx_mont_383_nonred
___
{
my @acc = map("%r$_","dx",15,"ax",12,"di","bp",	# output from __${pre}__mulx_mont_384
                      8..11,13,14);
