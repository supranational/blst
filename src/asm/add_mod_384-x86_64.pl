...
.type	mul_by_b_onE1,\@function,2,"unwind"
.align	32
mul_by_b_onE1:
.cfi_startproc
	push	%rbp
...
.type	mul_by_4b_onE1,\@function,2,"unwind"
.align	32
mul_by_4b_onE1:
.cfi_startproc
	push	%rbp
...
.type	mul_by_b_onE2,\@function,2,"unwind"
.align	32
mul_by_b_onE2:
.cfi_startproc
	push	%rbp
...
.cfi_end_prologue
	...
	lea	8*6($a_ptr), $b_org
	lea	8*6($r_ptr), $r_ptr
	call	__add_mod_384		# a->re + a->im
	call	__lshift_mod_384
	call	__lshift_mod_384

	mov	@acc[0], 8*0($r_ptr)	# ret->im
	mov	@acc[1], 8*1($r_ptr)
	mov	@acc[2], 8*2($r_ptr)
	mov	@acc[3], 8*3($r_ptr)
	mov	@acc[4], 8*4($r_ptr)
...
.type	mul_by_4b_onE2,\@function,2,"unwind"
.align	32
mul_by_4b_onE2:
.cfi_startproc
	push	%rbp
