...
pub const P2_Affine = struct {
    ...
    pub fn on_curve(self: *const P2_Affine) bool {
        return c.blst_p2_affine_on_curve(&self.point);
    }
    ...
    pub fn is_equal(self: *const P2_Affine, p: *const P2_Affine) bool {
        return c.blst_p2_affine_is_equal(&self.point, &p.point);
    }
    ...
    pub fn core_verify(self: *const P2_Affine, pk: *const P1_Affine,
                       ...
                       aug: ?[]const u8) ERROR {
        const opt = aug orelse &[_]u8{};
        const err = c.blst_core_verify_pk_in_g1(&pk.point, &self.point,
                                                hash_or_encode,
                                                @ptrCast(msg), msg.len,
                                                @ptrCast(DST), DST.len,
                                                @ptrCast(opt), opt.len);
    ...
    pub fn generator() P2_Affine {
        return P2_Affine {
            .point = c.blst_p2_affine_generator().*,
        };
    }

    pub fn to_jacobian(self: *const P2_Affine) P2 {
        var ret : P2 = undefined;
        c.blst_p2_from_affine(&ret.point, &self.point);
        return ret;
    }
};
...
pub const P2 = struct {
    ...
    pub fn deserialize(self: *P2, in: []const u8) ERROR {
        ...
        if (err == c.BLST_SUCCESS) {
            c.blst_p2_from_affine(&self.point, @ptrCast(&self.point));
        }
        return @as(ERROR, @enumFromInt(err));
    }
    ...
    pub fn serialize(self: *const P2) [P2_SERIALIZE_BYTES]u8 {
        var ret : [P2_SERIALIZE_BYTES]u8 = undefined;
        c.blst_p2_serialize(&ret[0], &self.point);
        return ret;
    }

    pub fn compress(self: *const P2) [P2_COMPRESS_BYTES]u8 {
        var ret : [P2_COMPRESS_BYTES]u8 = undefined;
        c.blst_p2_compress(&ret[0], &self.point);
        return ret;
    }

    pub fn public_key(sk: *const SecretKey) P2 {
        var ret : P2 = undefined;
        c.blst_sk_to_pk_in_g2(&ret.point, &sk.key);
        return ret;
    }
    ...
    pub fn on_curve(self: *const P2) bool {
        return c.blst_p2_on_curve(&self.point);
    }
    ...
    pub fn is_inf(self: *const P2) bool {
        return c.blst_p2_is_inf(&self.point);
    }

    pub fn is_equal(self: *const P2, p: *const P2) bool {
        return c.blst_p2_is_equal(&self.point, &p.point);
    }
    ...
    pub fn hash_to(msg: []const u8, DST: []const u8, aug: ?[]const u8) P2 {
        ...
        c.blst_hash_to_g2(&ret.point, @ptrCast(msg), msg.len,
                                      @ptrCast(DST), DST.len,
                                      @ptrCast(opt), opt.len);
        return ret;
    }
    ...
    pub fn encode_to(msg: []const u8, DST: []const u8, aug: ?[]const u8) P2 {
        ...
        c.blst_encode_to_g2(&ret.point, @ptrCast(msg), msg.len,
                                        @ptrCast(DST), DST.len,
                                        @ptrCast(opt), opt.len);
        return ret;
    }
    ...
    pub fn to_affine(self: *const P2) P2_Affine {
        var ret : P2_Affine = undefined;
        c.blst_p2_to_affine(&ret.point, &self.point);
        return ret;
    }

    pub fn generator() P2 {
        return P2 {
            .point = c.blst_p2_generator().*,
        };
    }
};
